class Student:
    ...
    def rate_lecturer(self, lecturer, course, grade_lecturer):
        if isinstance(lecturer, Lecturer) and course in self.course_progress and course in lecturer.course_list \
                and grade_lecturer <= 10:
    ...
    def __gt__(self, other):
        if self.average_grades() == f'У студента {self.name} нет оценок' or other.average_grades() == \
                f'У студента {self.name} нет оценок':
...
class Lecturer(Mentor):
    ...
    def __gt__(self, other):
        if self.average_grades_lec() == f'У лектора {self.name} нет оценок' or other.average_grades_lec() == \
                f'У лектора {self.name} нет оценок':
            return 'Невозможно сравнить'
        else:
    ...
    def average_lecturer(self, course):
        if course not in self.course_list:
            ...
            return f'Cредняя оценка лектора {self.name} за курс {course}: ' \
                   f'{round(sum(self.grades_lecturer[course]) / len(self.grades_lecturer[course]), 1)}'


def average_course_lec(lecturer_list, course):
    grades_list_lec = []
    for lecturer in lecturer_list:
        if course not in lecturer.course_list:
            print(f'Преподаватель {lecturer.name} не ведет лекции по курсу: {course}')
        elif course not in lecturer.grades_lecturer:
...
def average_course_stud(students_list, course):
    ...
    for student in students_list:
        if course not in student.course_progress:
...
student_2 = Student('Виктория', 'Викторова', 'жен')
student_2.add_course('python')
student_2.course_progress += ['git']

reviewer_1 = Reviewer('Артур', 'Артуров')
reviewer_1.course_list += ['python']

reviewer_2 = Reviewer('Виолетта', 'Виолеттова')
reviewer_2.course_list += ['git']

lecturer_1 = Lecturer('Константин', 'Константинов')
lecturer_1.course_list += ['python']

lecturer_2 = Lecturer('Евгения', 'Евгеньева')
...
print(f'Проверяющий 1: {reviewer_1}')
print(f'Проверяющий 2: {reviewer_2}')
